...
module_to_node<-function (module, annotation_table, col_module_id = "id_mgs", annotation_level){
    node_dict <- annotation_table %>% dplyr::select(all_of(col_module_id),
        !!sym(annotation_level)) %>% tibble::deframe()
...
graph_step<-function(data_with_annotation, col_module_id, annotation_level, type="fpkm"){
  if (type %in% c("coverage")){
    prev_df <- data_with_annotation %>%
      dplyr::mutate(prev = data_with_annotation %>% dplyr::select(-!!sym(annotation_level), -all_of(col_module_id)) %>% data.matrix() %>% `>`(0) %>% rowMeans())
    df <- prev_df %>% dplyr::filter(prev>0.15) %>% 
      dplyr::select(-!!sym(annotation_level), -all_of(col_module_id), -prev) %>% t() 
  ...
  if (type %in% c("fpkm")){
    counts <- OneNet::get_count_table(abund.table = data_with_annotation %>% dplyr::select(-!!sym(annotation_level)), sample_id=colnames(data_with_annotation), prev.min=0.15, verbatim=FALSE)$data
    G <- OneNet::new_synth_data(counts, n=50, plot=FALSE, verbose=FALSE)$G
    dimnames(G) <- list(colnames(counts), colnames(counts))
  }
  G <- G %>% as.data.frame() %>% 
    dplyr::mutate(!!sym(annotation_level):=module_to_node(rownames(G), annotation_table=data_with_annotation, col_module_id=col_module_id, annotation_level=annotation_level))
  G
}
```
...
simulate_by_prevalence <- function(data_with_annotation, prev_list, graph_file=NULL, col_module_id, annotation_level, sample_size=500, seed) {
  ...
  abund_table <- data_with_annotation %>% dplyr::select(-all_of(col_module_id), -!!sym(annotation_level))
  ...
  sim_one_prev <- function(prev) {
    ...
    df_sim <- OneNet::new_synth_data(df_counts$data, n=sample_size, graph=as.matrix(G %>% dplyr::select(-!!sym(annotation_level))), plot=FALSE, verbose=FALSE, seed=seed)
...
prev_for_selected_nodes <- function(data_with_annotation, graph_file, col_module_id, annotation_level, object_of_interest=NULL) {
  ...
  prev_df <- tibble::tibble(id_module = data_with_annotation %>% dplyr::pull(paste(col_module_id)),
                    prev = data_with_annotation %>% dplyr::select(-!!sym(annotation_level), -all_of(col_module_id)) %>% data.matrix() %>% `>`(0) %>% rowMeans())
  
  ## Extract edges starting or ending from the object of interest
  if (is.null(object_of_interest)) {object_index <- seq_along(G %>% dplyr::pull(!!sym(annotation_level)))} 
  else {object_index <- G %>% dplyr::pull(!!sym(annotation_level)) %>% stringr::str_detect(paste(object_of_interest))}
  
  G_object <- G[object_index, ] %>% dplyr::select(-!!sym(annotation_level)) 
...
norm_data<-function(data_with_annotation, col_module_id, prev_list=c(0.30), annotation_level, type){
  ...
  for (prev in prev_list){
    if (type %in% c("coverage")){
      #Arranging data
      df <- data_with_annotation %>% dplyr::select(-!!sym(annotation_level), -paste(col_module_id)) %>% t() 
...
#' Apply cv.glmnet() for a list of modules and for each prevalence level
#' 
...
test_that("intersections_table works", {
  ...
  table<-intersections_table(res_list=list(res_CRC_JPN, res_CRC_CHN, res_CRC_EUR), threshold=2, taxo, col_module_id="msp_id", "Escherichia coli")
