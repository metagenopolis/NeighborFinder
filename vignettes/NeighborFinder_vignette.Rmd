...
The displayed table indicates the precision and recall scores before and after applying the NeighborFinder method, for each level of prevalence tested.
NeighborFinder uses by default the level of prevalence of 30%. If the results from the table rendered by choose_prev_level() indicate better performance scores at another prevalence level, feel free to change the 'prev_level' parameter in the function apply_NeighborFinder().
...
choose_prev_level(data$CRC_JPN_CHN_EUR, "Escherichia coli", simulations, graph_file=G, col_msp_id="msp_id", seed = 20232024)
```
