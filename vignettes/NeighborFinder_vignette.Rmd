...
To do this step, generating a graph is needed. If your dataset is not of type "fpkm", please indicate one of the following equivalent words in 'type' argument: "comptage","couverture","coverage".
...
```{r graph_step}
# G <- graph_step(data$CRC_JPN_CHN_EUR, col_msp_id="msp_id")
...
simulations <- simulate_by_prevalence(data$CRC_JPN_CHN_EUR, prev_list=c(0.20,0.30), graph_file=G, col_msp_id="msp_id", seed=20232024)
choose_params_values(data$CRC_JPN_CHN_EUR, "Escherichia coli", simulations, filtering_list=c(10,20,30), graph_file=G, col_msp_id="msp_id", seed = 20232024) %>% as.data.frame()
```
...
intersections_table(res_list=list(res_CRC_JPN, res_CRC_CHN, res_CRC_EUR), threshold=2, taxo, col_msp_id="msp_id", "Escherichia coli") %>% kable()
```
